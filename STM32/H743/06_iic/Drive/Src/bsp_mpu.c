...
{
  // 保护SDRAM区域,共32M字节
  MPU_Set_Protection(0x0,                  // 基地址
                     MPU_REGION_SIZE_4GB,        // 长度
                     MPU_REGION_NUMBER0,          // NUMER2
                     MPU_REGION_NO_ACCESS,      // 全访问
                     MPU_ACCESS_SHAREABLE,    // 禁止共享
...
static void MPU_Set_Protection(u32 Address, u8 Size, u8 Number, u8 APer, u8 IsShar, u8 IsCach, u8 IsBuffer)
{
  ...
  MPU_InitStruct.SubRegionDisable = 0x87;                     /* 设置MPU区域的子区域 */
  MPU_InitStruct.TypeExtField = MPU_TEX_LEVEL0;               /* 指定MPU区域的TEX字段级别为0 */
  MPU_InitStruct.AccessPermission = APer;                     /* 设置MPU区域访问权限 */
  MPU_InitStruct.DisableExec = MPU_INSTRUCTION_ACCESS_DISABLE; /* 设置MPU区域的指令访问 */
