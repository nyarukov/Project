 ...
 * @param color 填充的颜色
 */
void LCD_Fill(uint16_t xStrat, uint16_t yStrat, uint16_t xEnd, uint16_t yEnd, uint16_t color)
{
    uint16_t i, j;
...
void LCD_DMA2D_Fill(uint16_t xStart,uint16_t yStart,uint16_t xEnd,uint16_t yEnd,uint16_t Color){
    ...
    BuffAdd=((uint32_t)SDRAM_LCD_BUF1+index(xStart,yStart)*2);
    OffLine= BABS(1024-(xEnd-xStart));
    
    __WRITE_REG(DMA2D->CR,0x00030000UL);                    
    
    __WRITE_REG(DMA2D->OMAR,(uint32_t)BuffAdd);                   
...
void LCD_DMA2D_Image(uint16_t xStart,uint16_t yStart,uint16_t xEnd,uint16_t yEnd,void *Image){
    ...
    __WRITE_REG(DMA2D->FGMAR,(uint32_t)Image);
