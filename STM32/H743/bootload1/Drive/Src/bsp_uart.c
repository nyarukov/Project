...
void Uart_ReceiveString(uint8_t* buffer, uint32_t bufferSize)
{
    uint32_t index = 0;
    while (index < bufferSize - 1)
    {
        while (!__HAL_UART_GET_FLAG(&huart1, UART_FLAG_RXNE));
        uint8_t receivedChar = huart1.Instance->RDR;
        if (receivedChar == '\n' || receivedChar == '\0')
        {
            break;
        }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    else
    {
        ...
        if ((Uart1.Rxbuf[Uart1.RxCnt - 1] == 0x0A) || (Uart1.Rxbuf[Uart1.RxCnt -1] == 0x0D))
        {
            Uart1.RXFlag = 1;
            if (Uart1.RXFlag != 0)
            {
