...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
    ...
    HAL_RCCEx_PeriphCLKConfig(&SPI_PerClk);
...
{
    HAL_SPI_Transmit(&hSPI, &_pBuffer, 1, 1000);
}
uint8_t Hard_SPI_Reveice(void)
{
    uint8_t RxBuffer;
    HAL_SPI_Receive(&hSPI, &RxBuffer, 1, 1000);
    return RxBuffer;
}
uint8_t Hard_SPI_TransmitReceive(uint8_t _pWBuffer)
{
    uint8_t RxBuffer;
    HAL_SPI_TransmitReceive(&hSPI, &_pWBuffer, &RxBuffer, 1, 1000);
    return RxBuffer;
}
