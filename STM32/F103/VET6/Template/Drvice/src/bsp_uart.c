...
#define __UART_REMAP(Com,Remap)                      \
    do                                               \
    {                                                \
        __CLEAR_BIT(AFIO->MAPR,(0x1 << (Com + 2)));  \
        __SET_BIT(AFIO->MAPR,(Remap << (Com + 2)));  \
    } while (0);                                     
...
static void Rs_485(COM_ID _Id, uint8_t Statu);
static Status Uart_SendData(COM_ID _Id, uint8_t *_pBuf, uint32_t _Len);
static void Uart_Sendch(COM_ID _Id, uint8_t ch);
static void Uart_SendNextByte(COM_ID _Id);
...
static void Uart_config(Uart_Config *Para)
{
    ...
    if(Uartx == USART1)
    {
        ...
        if (UART1_REMAP != UART_REMAP_NO)
        {
            __UART_REMAP(COM1,UART1_REMAP);
        }
    }
#endif
#if UART2_EN == 1
    else if(Uartx == USART2)
    {
        ...
        if (UART2_REMAP != UART_REMAP_NO)
        {
            __UART_REMAP(COM2,UART2_REMAP);
        }
    }
#endif
#if UART3_EN == 1
    else if(Uartx == USART3)
    {
        ...
        if (UART3_REMAP != UART_REMAP_NO)
        {
            __UART_REMAP(COM3,UART3_REMAP);
        }
    }
#endif
#if UART4_EN == 1
    else if(Uartx == UART4)
    {
        ...
        if (UART4_REMAP != UART_REMAP_NO)
        {
            __UART_REMAP(COM4,UART4_REMAP);
        }
    }
#endif
#if UART5_EN == 1
    else if(Uartx == UART5)
    {
        ...
        if (UART5_REMAP != UART_REMAP_NO)
        {
            __UART_REMAP(COM5,UART5_REMAP);
        }   
    }
#endif
    ...
    else if(Uartx == UART6)
    {
        ...
        __UART_REMAP(COM6,UART6_REMAP);
    }
    else if(Uartx == UART7)
    {
        RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART7, ENABLE);
        NVIC_InitStructure.NVIC_IRQChannel = UART7_IRQn;
    _   _UART_REMAP(COM7,UART7_REMAP);
    }
    else if(Uartx == UART7)
    {
        RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART8, ENABLE);
        NVIC_InitStructure.NVIC_IRQChannel = UART8_IRQn;
        __UART_REMAP(COM8,UART5_REMAP);
    }
    #endif
    ...
    IO_Init(Para->Para.Tx_IO, IO_Mode_AF_PP, IO_Mode_NoPull, IO_Speed_50MHz);
    IO_Init(Para->Para.Rx_IO, IO_Mode_IN_FLOATING, IO_Mode_IPU, IO_Speed_50MHz);
    if (Para->Para.Rs_IO != UART_RS485_IO_NO)
    {
        IO_Init(Para->Para.Rs_IO, IO_Mode_Out_PP, IO_Mode_NoPull, IO_Speed_50MHz);
        IO_Wrtie(Para->Para.Rs_IO, 0);
    }
...
void Uart_Proc(COM_ID _Id)
{
    ...
    {
        if(Config[_Id].Para.Rs_IO != UART_RS485_IO_NO)
        {
            Rs_485(_Id, 1);
        }
