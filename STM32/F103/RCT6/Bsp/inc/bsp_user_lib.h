...
// GPIO复位位宏
#define __GPIO_RESET_BIT(REG, MASK) __SET_BIT((REG->BRR), (1 << MASK))
// GPIO输入模式宏
#define __GPIO_INPUT(REG, MASK) __CLEAR_BIT((REG->CRL), (1 << (MASK * 2)))
// GPIO输出模式宏
#define __GPIO_OUTPUT(REG, MASK) __SET_BIT((REG->CRL), (1 << (MASK * 2)))
//  GPIO输入读取宏
...
#define __GPIO_WRITE_BIT(REG, MASK, VAL) ((VAL) ? __SET_BIT((REG->BSRR), (1 << MASK)) : __SET_BIT((REG->BRR), (1 << MASK)))
